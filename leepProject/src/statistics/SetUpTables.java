...
public class SetUpTables {
    ...
    public static final int[] crnsM201101 = { 38588, 38636, 38764, 38765, 38824, 38832, 38900, 39021 };
    public static final int[] timesM201101 = { 4, 4, -1, -1, 4, 1, 2, 4 };
    public static final int[] daysM201101 = { 2, 3, 1, 1, 3, 3, 4, 3 };
    public static final int[] crnsM201109 = { 20180, 20544, 21062, 21331, 21489, 21602 };
    public static final int[] timesM201109 = { 4, 3, 4, 3, 2, 2 };
    public static final int[] daysM201109 = { 3, 3, 2, 1, 2, 4 };
    public static final int[] crnsM201201 = { 30272, 30319, 30482, 30554, 31400, 31791 };
    ...
    public static final int[] crnsM201301 = { 30272, 30482, 30521, 30554, 32313, 32357 };
    ...
    public static final int[] crnsO201109 = { 20167, 21298, 21331, 20409, 20415, 21356, 21365 };
	...
	batchUpdateTimes("courseswfins201101", crnsM201101, daysM201101, timesM201101);
	batchUpdateTimes("courseswfins201109", crnsM201109, daysM201109, timesM201109);
	batchUpdateTimes("courseswfins201201", crnsM201201, daysM201201, timesM201201);
	batchUpdateTimes("courseswfins201209", crnsM201209, daysM201209, timesM201209);
	batchUpdateTimes("courseswfins201301", crnsM201301, daysM201301, timesM201301);

	batchUpdateTimes("courseswfins201101", crnsO201101, daysO201101, timesO201101);
	batchUpdateTimes("courseswfins201109", crnsO201109, daysO201109, timesO201109);
	batchUpdateTimes("courseswfins201201", crnsO201201, daysO201201, timesO201201);
	batchUpdateTimes("courseswfins201209", crnsO201209, daysO201209, timesO201209);
	batchUpdateTimes("courseswfins201301", crnsO201301, daysO201301, timesO201301);
	...
	while (rs.next()) {
	    ...
	    String query1 = "SELECT FinalDay, FinalTime FROM " + courses + " WHERE CourseCRN = '" + crn + "';";
	    prl(query1);
		...
		String query = "UPDATE " + studs + " SET FinalDay = '" + day + "', FinalTime = '" + time
			+ "' WHERE CourseCRN = '" + crn + "';";
    ...
    public static void batchUpdateTimes(String table, int[] crns, int[] days, int[] times) throws SQLException {
	Statement st = con.createStatement();
	for (int i = 0; i < crns.length; i++) {
	    String query = "UPDATE " + table + " SET FinalDay = '" + days[i] + "', FinalTime = '" + times[i]
		    + "' WHERE CourseCRN = '" + crns[i] + "';";
	    prl(query);
    ...
    public static void assignGeneralSFinals(String semester) throws SQLException {
	Statement st = con.createStatement();
	Statement st2 = con.createStatement();
	...
	while (rs.next()) {
	    String crn = rs.getString(1);
	    String getBlocks = "SELECT t1.block, t1.day FROM " + table1 + " AS t1, " + table
		    + " AS t2 WHERE REPLACE(t1.pattern,' ','') = REPLACE(t2.Days,' ','')"
		    + " AND t1.start = t2.MeetBeginTime AND t1.end=t2.MeetEndTime AND t2.CourseCRN = '" + crn + "';";
	    //prl(getBlocks);
	    ...
	    String query = "UPDATE " + table + " SET finalDay ='" + day + "', finalTime ='" + block
		    + "' WHERE CourseCRN = '" + crn + "';";
	    //prl(query);
	    rs2.close();
	    st2.executeUpdate(query);
	...
	String getCRNs = "SELECT DISTINCT CourseCRN FROM " + table + " WHERE CourseCRN NOT IN (SELECT CourseCRN FROM "
		+ table + " GROUP BY CourseCRN HAVING COUNT(CourseCRN) > 1)";
	prl(getCRNs);
	ResultSet rs = st.executeQuery(getCRNs);
	while (rs.next()) {
	    String crn = rs.getString(1);
    ...
    public static void removeStrangeDayThings(String semester) throws SQLException {
	if (true) {
	    Statement st = con.createStatement();
	    Statement st2 = con.createStatement();
	    String table = CWF + semester;

	    String select = "SELECT DISTINCT t1.CourseCRN FROM " + table + " AS t1 ," + table
		    + " AS t2 WHERE t1.CourseCRN IN (SELECT CourseCRN FROM " + table
	    ...
	    while (rs.next()) {
		String crn = rs.getString(1);
		String getDays = "SELECT Days, " + table + ".* FROM " + table + " WHERE CourseCRN = '" + crn + "';";
		...
		String query = "UPDATE " + table + " AS t SET Days = '" + days + "' WHERE t.CourseCRN = '" + crn + "';";
		st2.executeUpdate(query);

		String query2 = "CREATE TABLE temp AS (SELECT DISTINCT * FROM " + table + ");";
	...
	if (true) {
		...
		String condition = cond(swf, "MeetBeginTime", sch, "start") + "AND"
			+ cond(swf, "MeetEndTime", sch, "end") + "AND " + join(swf, "StudentIDNo") + " = '" + id
			+ "' AND REPLACE(" + join(swf, "Days") + ",' ','') = REPLACE(" + join(sch, "pattern")
			+ ",' ','');";
	...
	String query = "CREATE TABLE " + name + semester + " AS (SELECT " + STUDS + semester + ".* " + "FROM " + STUDS
		...
		+ ".CourseCRN);";
	String query2 = "ALTER TABLE " + name + semester + " ADD COLUMN finalDay INT NOT NULL DEFAULT '-1';";
	String query3 = "ALTER TABLE " + name + semester + " ADD COLUMN finalTime INT NOT NULL DEFAULT '-1';";
	prl(query);
	...
	String drop = "DROP TABLE IF EXISTS " + name + semester + ";";
	String query = "CREATE TABLE " + name + semester + " AS (SELECT " + COURSES + semester + ".* " + "FROM "
		+ COURSES + semester + ", " + FINS + semester + " WHERE " + COURSES + semester + ".CourseCRN = " + FINS
		+ semester + ".CourseCRN);";
	String query2 = "ALTER TABLE " + name + semester + " ADD COLUMN finalDay INT NOT NULL DEFAULT '-1';";
	String query3 = "ALTER TABLE " + name + semester + " ADD COLUMN finalTime INT NOT NULL DEFAULT '-1';";
	prl(query);
